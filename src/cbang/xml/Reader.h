...
namespace cb {
  namespace XML {
    ...
    public:
      Reader(bool skipRoot = true);
      virtual ~Reader();

      void read(const std::string &filename, Handler *handler);
      void read(std::istream &stream, Handler *handler);
    ...
    protected:
      void push(Handler *handler, HandlerFactory *factory);
      bool pop();
      Handler &get();
    };
  }
